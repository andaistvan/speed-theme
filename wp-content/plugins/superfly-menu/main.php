...
//add_filter( 'the_content', 'sf_main_html' );
...
//	$ismobile = sf_is_mobile();
	...
	if ($sf_show) {
        ...
        $script_url = $options['sf_dev'] === 'yes' ? '/js/superfly-menu.js' : '/js/superfly-menu.min.js';
		...
		if (!empty($options['sf_linkedin'])) $social['linkedin'] = $options['sf_linkedin'];
		...
		wp_localize_script( 'sf_main', 'SF_Opts', array(
				...
				'width_panel_1' => ($options['sf_sidebar_style'] === 'static' && $options['sf_iconbar'] == 'yes' ? 100 : $options['sf_width_panel_1']),
